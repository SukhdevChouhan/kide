...
public class IssueController {
	...
	public String handleTaskCreateForm(@Valid @ModelAttribute("form") IssueCreateForm form, BindingResult bindingResult) {
		...
		try {
			final UserRole userRole = this.userManager.getUserRole(form.getIssue_name());
			...
			// of them.
			LOGGER.warn("Exception occurred when trying to save the task, assuming duplicate task id", e);
	...
	public String handleTaskEditForm(@Valid @ModelAttribute("task") IssueCreateForm form, BindingResult bindingResult) {
		System.out.println("******b***********" + form);
		LOGGER.debug("Processing task edit form={}, bindingResult={}", form, bindingResult);
		if (bindingResult.hasErrors()) {
			// failed validation
